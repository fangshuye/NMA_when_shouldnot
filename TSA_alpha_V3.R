...
table_with_prev <- foreach (rep=1:nrep, .combine = rbind)%dopar%{
  ...
  sig_unadj <- ifelse(p_value_2 < 0.05,1,0)
  sig <- ifelse(z1>c1 | z2>c2,1,0)
  return(c(p_value_1,sig_unadj,sig))
}
...
error_rate_1 <- apply(BE_1, 2, mean) # Two Type I error rate (without adjustment and with)
error_rate_2 <- apply(BE_2, 2, mean) # Two Type I error rate (without adjustment and with)
error_rate_3 <- apply(BE_3, 2, mean) # Two Type I error rate (without adjustment and with)
...
cat("Total number of new study:", nrow(BE_1), "\n")
cat("Type I error without adjustment:", error_rate_1[1], "\n")
cat("Type I error with adjustment", error_rate_1[2], "\n")
cat( "When 0.05 < p-value < 0.1 is the criterion for promising result","\n")
cat("Total number of new study:", nrow(BE_2), "\n")
cat("Type I error without adjustment:", error_rate_2[1], "\n")
cat("Type I error with adjustment", error_rate_2[2], "\n")
cat( "=====================================================","\n")
cat( "When the new trial will be conducted regardless of the previous result","\n")
cat("Total number of new study:", nrow(BE_3), "\n")
cat("Type I error without adjustment:", error_rate_3[1], "\n")
cat("Type I error with adjustment", error_rate_3[2], "\n")
cat( "=====================================================","\n")
sink()
